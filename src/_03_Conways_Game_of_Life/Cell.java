...
public class Cell implements Drawable{
	...
	public void liveOrDie(int numNeighbors) {
		if(numNeighbors < 2 && isAlive) {isAlive = false;}
		if((numNeighbors == 2 || numNeighbors == 3) && isAlive) {isAlive = true;}
		if(numNeighbors > 3) {isAlive = false;}
		if(numNeighbors == 3 && isAlive == false) {isAlive = true;}
	}
	...
	public void draw(Graphics g) {
		...
		if(isAlive) {
			g.setColor(Color.getHSBColor(new Random().nextInt(), new Random().nextFloat(), new Random().nextFloat()));
